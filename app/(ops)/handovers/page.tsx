...
import { ArrowLeftRight, ArrowUpRight, Clock, CheckCircle2, XCircle } from "lucide-react";
...
export default async function HandoversPage() {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      id, status, odometer_km, created_at,
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px] mx-auto">
      <div>
        <h1 className="text-2xl lg:text-3xl font-bold text-ink-900 tracking-tight">Vehicle handovers</h1>
        <p className="text-sm text-ink-500 mt-1">
          Driver-to-driver handovers — both parties complete a checklist and agree on condition.
        </p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <Tile icon={Clock} tone="amber" label="Pending" value={stats.pending} />
      ...
      {list.length === 0 ? (
        <div className="rounded-2xl bg-white border border-ink-200/70 py-16 text-center">
          <div className="inline-flex h-14 w-14 rounded-2xl bg-ink-100 items-center justify-center mb-3">
            <ArrowLeftRight className="h-6 w-6 text-ink-400" />
          </div>
          <p className="text-sm font-semibold text-ink-900">No handovers yet</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-ink-100">
              {list.map((r) => (
                <tr key={r.id} className="hover:bg-ink-50/40 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-ink-700">{r.from_driver?.profiles?.full_name ?? "—"}</td>
                  <td className="px-6 py-4 text-ink-700">{r.to_driver?.profiles?.full_name ?? "—"}</td>
...
export function StatusPill({ status }: { status: Status }) {
  const cfg = {
    ...
    accepted: { label: "Accepted", bg: "bg-emerald-50 text-emerald-700 border-emerald-200", dot: "bg-emerald-500" },
    rejected: { label: "Rejected", bg: "bg-rose-50 text-rose-700 border-rose-200", dot: "bg-rose-500" },
    cancelled: { label: "Cancelled", bg: "bg-ink-100 text-ink-600 border-ink-200", dot: "bg-ink-400" },
  }[status];
  return (
