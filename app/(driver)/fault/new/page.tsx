...
import { FaultReportForm } from "@/components/driver/FaultReportForm";
import type { CountryCode } from "@/types/domain";

export const dynamic = "force-dynamic";

export default async function DriverNewFaultPage() {
      ...
      <div className="p-4 pt-8">
        <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 flex gap-3">
          ...
          <p className="text-sm text-amber-700">
            Your driver profile is not set up yet. Contact your fleet manager.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="p-4 pt-8 space-y-4">
        ...
        <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 flex gap-3">
          <AlertCircle className="h-5 w-5 text-amber-600 shrink-0" />
          <p className="text-sm text-amber-700">
            You have no vehicle assigned. Speak to your fleet manager.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-4 pt-6 space-y-5">
      ...
      <header>
        <h1 className="text-2xl font-bold text-ink-900">Report a fault</h1>
        <p className="text-sm text-ink-500 mt-1">Tell us about a problem with the vehicle</p>
      </header>
      ...
      <FaultReportForm vehicle={vehicle} activeTripId={activeTrip?.id ?? null} />
    </div>
  );
}
